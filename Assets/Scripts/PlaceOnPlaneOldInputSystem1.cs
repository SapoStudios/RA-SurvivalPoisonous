...
public class PlaceOnPlaneOldInputSystem1 : MonoBehaviour
{
    /// <summary>
        ...
        if (aRRaycastManager.Raycast(Input.GetTouch(0).position, hits, TrackableType.PlaneWithinPolygon))
        {
            ...
            if (spawnedObject == null)
            {
                for (int i = 0; i < spawnedObject.Length; i++) {
                    spawnedObject[i] = Instantiate(placedPrefab, hitPose.position, hitPose.rotation); 
                }
            }
            /*else
            {
            ...
            for (int i = 0; i < spawnedObject.Length; i++)
            {
                Vector3 lookPos = Camera.main.transform.position - spawnedObject[i].transform.position;
                lookPos.y = 0;
                spawnedObject[i].transform.rotation = Quaternion.LookRotation(lookPos);
            }
        }
    }
}
